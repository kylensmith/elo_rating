...
class EloRating::Match
  ...
  class Player
  # :nodoc:
    attr_reader :rating, :place, :match, :placerid
    def initialize(match:, rating:, place: nil, winner: nil, playerid: nil)
      validate_attributes!(rating: rating, place: place, winner: winner, playerid: id)
      @match = match
      @rating = rating
      @place = place
      @winner = winner
